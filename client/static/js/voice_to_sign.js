...
async function convertTextToSigns(text) {
  ...
  try {
    // Use apiRequest from main.js which handles API_BASE_URL
    const data = await apiRequest("/api/text_to_signs", {
      method: "POST",
      body: JSON.stringify({ text: text.trim() }),
    });

    if (data.status === "success") {
      // Update image paths to use local static folder
      currentSignSequence = data.signs.map(sign => ({
        ...sign,
        image: sign.type === 'space' ? null : `static/images/${sign.char}.jpg`
      }));
      displaySignSequence();
      updateStatistics();
